...
import com.example.amusetravelproejct.dto.response.AdminPageResponse.categoryRegister;
...
public class CategoryService {
    ...
    // 새로운 카테고리 등록
    public AdminPageResponse.categoryRegister processRegisterCategory(
        AdminPageRequest.categoryRegister categoryRegisterDto, AdminService adminService,
        UtilMethod utilMethod) {
        Admin admin = adminService.getAdminByAdminId(categoryRegisterDto.getCreatedBy())
        ...
        Category category = Category.builder()
            ...
            .imgUrl(utilMethod.getImgUrl(categoryRegisterDto.getBase64Data(),
                categoryRegisterDto.getFileName()))
            .build();
    ...
    public AdminPageResponse.categoryEdit processEditCategory(
        AdminPageRequest.categoryEdit categoryEditDto, AdminService adminService,
        UtilMethod utilMethod) {
        ...
        Admin admin = adminService.getAdminByAdminId(categoryEditDto.getUpdatedBy())
            .orElseThrow(() -> new CustomException(ErrorCode.ADMIN_NOT_FOUND));

        if (categoryEditDto.getFileName() != "") {
            category.setImgUrl(utilMethod.getImgUrl(categoryEditDto.getBase64Data(),
                categoryEditDto.getFileName()));
        }
        ...
        return new AdminPageResponse.categoryEdit(category.getId(), category.getCategory_name(),
            category.getImgUrl(), category.getSequence(), category.getMainDescription(),
            category.getSubDescription(), category.getCreatedDate(),
            category.getAdmin().getAdminId(), category.getModifiedDate(),
            category.getUpdateAdmin().getAdminId());
    }
    ...
    public List<AdminPageResponse.categorySequence> processGetCategorySequence() {
        ...
        for (Category category : categoryList) {
            categorySequenceList.add(new AdminPageResponse.categorySequence(category.getId(),
                category.getCategory_name(), category.getSequence(), category.getCreatedDate(),
                category.getAdmin().getAdminId(),
                category.getUpdateAdmin() == null ? null : category.getModifiedDate(),
